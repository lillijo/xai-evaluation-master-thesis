...
We generate datasets by sampling the spurious-to-core feature ratio $a$ in 0.02 steps and training 4 \todo{how many iterations} models with different random initializations on the same dataset. In total, 224 \todo{how many models total} models are trained. The training dataset contains 30\% of samples and the validation set the other 70\%. Experimentally, much fewer samples seemed to be enough to achieve high accuracies, however there is no need to fear overfitting as we are aware of the true causal generative process. 

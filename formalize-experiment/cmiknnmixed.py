...
# Author: Oana Popescu and Jakob Runge <jakob@jakob-runge.com>
#
    ...
    **kwargs :
    ...
    def _transform_mixed_data(self, array, type_mask=None, add_noise=False):
        ...
        if add_noise:
            ...
            array[continuous_idxs, :] += (
                1e-6
                * array[continuous_idxs, :].std(axis=1).reshape(cont_dim, 1)
                * self.random_state.random(
                    (
                        array[continuous_idxs, :].shape[0],
                        array[continuous_idxs, :].shape[1],
                    )
                )
            )
        if self.transform == "standardize":
            array[continuous_idxs, :] = self._standardize_array(
                array[continuous_idxs, :], cont_dim
            )
        elif self.transform == "scale":
            array[continuous_idxs, :] = self._scale_array(
                array[continuous_idxs, :], minmax=self.scale_range
            )
    ...
    def _transform_to_one_hot_mixed(self, array, xyz, type_mask, zero_inf=False):
        ...
        for i in range(len(discrete_idx_list)):
            if discrete_idx_list[i] == 1:
                ...
                narray = np.concatenate(
                    [narray[:, :i], encoder_df, narray[:, i + 1 :]], axis=-1
                )

                nxyz = np.concatenate([nxyz[:i], xyz_val, nxyz[i + 1 :]])
                ntype_mask = np.concatenate(
                    [
                        ntype_mask[:, :i],
                ...
                categories = np.delete(categories, categories == 0.0)
                ...
                narray = np.concatenate(
                    [narray[:, :i], cont_column, encoder_df, narray[:, i + 1 :]],
                    axis=-1,
                )
    ...
    def get_significance(
        self, val, array, xyz, T, dim, type_mask=None, sig_override=None
    ):
        """
    ...
    def get_smallest_cluster_size(self, array, type_mask=None):
        ...
        num_xyz_classes = [
            np.unique(array[:, index])
            for index in range(len(discrete_idx_list))
            if (discrete_idx_list[index] == 1)
        ]
        ...
        if len(xyz_cartesian_product) > 0:
            for i, entry in enumerate(xyz_cartesian_product):
                current_array = array[
                    np.sum(array[:, discrete_xyz_idx] == entry, axis=-1)
                    == len(discrete_xyz_idx)
                ]
    ...
    @jit(forceobj=True)
    def _get_nearest_neighbors_zeroinf_onehot(self, array, xyz, knn, type_mask=None):
        ...
        narray, nxyz, ntype_mask, discrete_idx_list = self._transform_to_one_hot_mixed(
            array, xyz, type_mask, zero_inf=True
        )
        ...
        neighbors = tree_xyz.query(
            narray, k=knn + 1, p=np.inf, distance_upper_bound=9999999
        )
        ...
        k_tilde = [
            len(neighbors_radius_xyz[i]) - 1
            if len(neighbors_radius_xyz[i]) > 1
            else len(neighbors_radius_xyz[i])
            for i in range(len(neighbors_radius_xyz))
        ]
    ...
    def get_dependence_measure_MSinf(self, array, xyz, type_mask=None):
        ...
        if self.knn < 1:
            ...
            raise ValueError(
                "MSinf needs knn value as percentage (value < 1), not number of neighbors!"
            )

        knn_tilde, k_xz, k_yz, k_z = self._get_nearest_neighbors_zeroinf_onehot(
        ...
        if self.max_with_0 and val < 0.0:
            val = 0.0

        return val, non_zero_count

    @jit(forceobj=True)
    def _get_nearest_neighbors_MS_one_hot(self, array, xyz, knn, type_mask=None):
        ...
        neighbors_radius_xyz = tree_xyz.query_ball_point(
            narray, epsarray, p=np.inf, workers=self.workers
        )
    ...
    def get_dependence_measure_MS(self, array, xyz, type_mask=None):
        ...
        knn_tilde, k_xz, k_yz, k_z = self._get_nearest_neighbors_MS_one_hot(
            array=array, xyz=xyz, knn=knn, type_mask=type_mask
        )
    ...
    def _compute_entropies_for_discrete_entry(
        self,
        array,
        discrete_values,
        discrete_idxs,
        ...
        current_array = array[
            np.sum(array[:, discrete_idxs] == discrete_values, axis=-1)
            == len(discrete_idxs)
        ]
        ...
        if len(continuous_idxs) > 0 and T > knn:
            val_continuous_entropy = self._compute_continuous_entropy(
                current_array[:, continuous_idxs], knn
            )
        else:
            val_continuous_entropy = 0.0
    ...
    def _compute_continuous_entropy(self, array, knn):
        ...
        epsarray = tree.query(
            array, k=[knn + 1], p=np.inf, workers=self.workers, eps=0.0
        ...
        if num_non_zero == 0:
            ...
            avg_dist = (
                float(array.shape[-1])
                / float(num_non_zero)
                * np.sum(np.log(2 * epsarray))
            )
            cmi_hat = special.digamma(num_non_zero) - special.digamma(knn) + avg_dist

        return cmi_hat

    def get_dependence_measure_ZMADG(self, array, xyz, type_mask=None):
        ...
        if self.knn > 1:
            raise ValueError(
                "ZMADG needs knn value as percentage (value < 1), not number of neighbors!"
            )
        else:
            knn = self.knn
        ...
        num_xz_classes = [
            np.unique(array[:, xz_indices][:, index])
            for index in range(len(discrete_xz_indices))
            if (discrete_xz_indices[index] == 1)
        ]
        num_yz_classes = [
            np.unique(array[:, yz_indices][:, index])
            for index in range(len(discrete_yz_indices))
            if (discrete_yz_indices[index] == 1)
        ]
        num_z_classes = [
            np.unique(array[:, z_indices][:, index])
            for index in range(len(discrete_z_indices))
            if (discrete_z_indices[index] == 1)
        ]
        num_xyz_classes = [
            np.unique(array[:, index])
            for index in range(len(discrete_idx_list))
        ...
        if len(xyz_cartesian_product) > 0:
            ...
            for i, entry in enumerate(xyz_cartesian_product):
                (
                    xyz_cont_entropy,
                    xyz_disc_entropy,
                ) = self._compute_entropies_for_discrete_entry(
                    array, entry, discrete_xyz_idx, continuous_xyz_idx, T, knn
                )
        ...
        if len(z_cartesian_product) > 0:
            ...
            for i, entry in enumerate(z_cartesian_product):
                (
                    ...
                    array[:, z_indices], entry, discrete_z_idx, continuous_z_idx, T, knn
                )
    ...
    def get_dependence_measure(self, array, xyz, type_mask=None):
        ...
        if self.estimator == "MS":
            return self.get_dependence_measure_MS(array, xyz, type_mask)
        elif self.estimator == "ZMADG":
            return self.get_dependence_measure_ZMADG(array, xyz, type_mask)
        elif self.estimator == "MSinf":
            return self.get_dependence_measure_MSinf(array, xyz, type_mask)
        else:
    ...
    def _generate_random_permutation(self, array, neighbors, x_indices, type_mask):
        ...
        restricted_permutation = self.get_restricted_permutation(
            T=T, shuffle_neighbors=self.shuffle_neighbors, neighbors=n, order=order
        )

        # print(restricted_permutation)
    ...
    @jit(forceobj=True)
    def compute_perm_null_dist(self, array, xyz, type_mask=None):
        ...
        # be the point itself
        valid_neighbors = np.multiply(
            valid_neighbors,
            np.expand_dims(np.arange(valid_neighbors.shape[0]), axis=-1),
        )
        ...
        for sam in range(self.sig_samples):
            ...
            array_shuffled, type_mask_shuffled = self._generate_random_permutation(
                array,
                valid_neighbors,
                x_indices=np.where(xyz == 0)[0],
                type_mask=type_mask,
            ...
            null_dist[sam], _ = self.get_dependence_measure(
                array_shuffled.T, xyz, type_mask=type_mask_shuffled.T
            )
        return null_dist

    # @jit(forceobj=True)
    def get_shuffle_significance(
        self, array, xyz, value, return_null_dist=False, type_mask=None
    ):
        ...
        if len(z_indices) > 0 and self.shuffle_neighbors < T:
            null_dist = self.compute_perm_null_dist(array, xyz, type_mask)
    ...
    def _get_shuffle_dist(
        self, array, xyz, sig_samples, sig_blocklength=None, type_mask=None, verbosity=0
    ):
        ...
        for sam in range(sig_samples):
            ...
            type_x_shuffled = np.zeros(
                (dim_x, n_blks * sig_blocklength), dtype=array.dtype
            )
            ...
            for i, index in enumerate(x_indices):
                for blk in range(sig_blocklength):
                    ...
                    type_x_shuffled[i, blk::sig_blocklength] = type_mask[
                        index, blk_starts + blk
                    ]
            ...
            if tail.shape[1] > 0:
                ...
                type_x_shuffled = np.insert(
                    type_x_shuffled, insert_tail_at, tail_type.T, axis=1
                )
            ...
            null_dist[sam], _ = self.get_dependence_measure(
                array=array_shuffled, xyz=xyz, type_mask=type_mask_shuffled
            )
